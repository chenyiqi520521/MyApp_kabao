...
import com.apicloud.landy.LandySwipService;
...
public class SwipActivity extends Activity implements OnClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(UZResourcesIDFinder.getResLayoutID("layout_swip"));
		initView();
	...
	void checkBlueTooth(){
		...
		if(chose_device.equals(UICommon.BBPOS_IC_DEVICE)||chose_device.equals(UICommon.CFT_DEVICE)){
			...
			//清空地址
			SwipActivity.this.getSharedPreferences(BaseActivity.KALAI_COMMON_SAVE,0).edit().putString(BaseActivity.CHOSED_BLUETOOTH_ADDRESS,"").commit();
			SwipActivity.this.getSharedPreferences(BaseActivity.KALAI_COMMON_SAVE,0).edit().putString(BaseActivity.CHOSED_LAST_DEVICE_NAME,"").commit();
			//更新最新设备
			SwipActivity.this.getSharedPreferences(BaseActivity.KALAI_COMMON_SAVE,0).edit().putString(BaseActivity.CHOSED_LAST_DEVICE_TYPE, chose_device).commit();
	    //如果不是卡头设备
		}else{
			//如果上个设备不同
			String last_device=SwipActivity.this.getSharedPreferences(BaseActivity.KALAI_COMMON_SAVE,0).getString(BaseActivity.CHOSED_LAST_DEVICE_TYPE, "");
			...
			//更新最新设备
			SwipActivity.this.getSharedPreferences(BaseActivity.KALAI_COMMON_SAVE,0).edit().putString(BaseActivity.CHOSED_LAST_DEVICE_TYPE, chose_device).commit();
			String address=SwipActivity.this.getSharedPreferences(BaseActivity.KALAI_COMMON_SAVE,0).getString(BaseActivity.CHOSED_BLUETOOTH_ADDRESS,"");
		    String bluename=SwipActivity.this.getSharedPreferences(BaseActivity.KALAI_COMMON_SAVE,0).getString(BaseActivity.CHOSED_LAST_DEVICE_NAME,"");
			//如果地址大于0，就初始化设备
			if(address.length()>0){
		...
		mainHandler=new Handler(){
			...
			public void handleMessage(Message msg) {
				...
				if(msg.what==FINISH_ENCRY_KEY_SUCCESS){
				    ...
				    api.closeDev();
				    //返回给上个页面
				    Intent responseIntent=new Intent(SwipActivity.this,BaseActivity.class);
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		if(keyCode==KeyEvent.KEYCODE_BACK){
			try {
				api.closeDev();
				Intent responseIntent=new Intent(SwipActivity.this,BlueToothSearchActivity.class);
				setResult(RESPONSE_WORK_FAIL, responseIntent);
				finish();
			} catch (Exception e) {
				// TODO: handle exception
	...
	private void loadBBPOS_IC(Handler handler){
		...
		api=new MybbosIcService(getApplicationContext(), new SwipUiHandler(SwipActivity.this){
			@Override
	...
	private void loadCFT(Handler handler){
		...
		api=new myCsSwipService(getApplicationContext(), new SwipUiHandler(SwipActivity.this){
	...
	private void loadAfSDK(Handler handler){
		...
		api=new afSwipService(getApplicationContext(), new SwipUiHandler(SwipActivity.this){
	...
	private void loadWftSDK(Handler handler){
		...
		params.put("whatDo", whatToDo);
		params.put("amount", amount+"");
		api=new wftService(SwipActivity.this, new SwipUiHandler(SwipActivity.this){
			@Override
			protected void showText(Message msg) {
				// TODO Auto-generated method stub
				super.showText(msg);
	...
	private void loadLandySDK(Handler handler){
		...
		params.put("whatDo", whatToDo);
		params.put("amount", amount+"");
		api=new LandySwipService(SwipActivity.this,new SwipUiHandler(SwipActivity.this){
			...
			protected void showText(Message msg) {
				...
				String tipMsg=(String) msg.obj;
				 Log.v("landy1",tipMsg+"");
				//如果是成功连接就切换背景
				if(tipMsg.equals(LandySwipService.CONNECTED_SUCCESS)){
					im_shuaka.setBackgroundResource(UZResourcesIDFinder.getResDrawableID("ly_cash2_landy"));
				}
	...
	private void Tip(final String msg){
		runOnUiThread(new Thread(){
			...
			public void run() {
				// TODO Auto-generated method stub
				super.run();
				Toast.makeText(SwipActivity.this, msg, Toast.LENGTH_SHORT).show();
			}
		});
	...
	public void onClick(View v) {
		...
		if(v == ModelTopUpDialog.txt_ok){
			...
			if(password==null||password.length()<=0){
				Toast.makeText(SwipActivity.this, "密码不能为空", Toast.LENGTH_SHORT).show();
				return;
			}
		...
		if(v==btn_help){
			this.startActivity(new Intent(SwipActivity.this, WebViewActivity.class));
		}
		if(v==iv_back){
			finish();
		}
				...
				builder.setSingleChoiceItems(bluetoothNames, 0, new AlertDialog.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						...
						try {
							...
							SwipActivity.this.getSharedPreferences(BaseActivity.KALAI_COMMON_SAVE,0).edit().putString(BaseActivity.CHOSED_LAST_DEVICE_NAME,bluename).commit();
							} catch (Exception e) {
