 ...
 * @see 钱包充值界面
 */
public class QueryMoneyActivity extends BaseActivity implements OnClickListener {
	ImageButton ib_return;// 返回按钮
	TextView ed_crad;// 卡号
	EditText ed_crad_amount;// 金额
	EditText ed_crad_pwd;// 金额
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		int layoutId = UZResourcesIDFinder.getResLayoutID("query_money");
		if (layoutId > 0) {
			setContentView(layoutId);
		}
		initView();
		topUpDialog = new TopUpDialog(QueryMoneyActivity.this);
	...
	private void initView() {
		...
		ed_crad_pwd = (EditText) findViewById(UZResourcesIDFinder.getResIdID("ed_crad_pwd"));
        ...
        ll_pwd=(LinearLayout) findViewById(UZResourcesIDFinder.getResIdID("ll_pwd"));
		...
		rad_four.setTag("D");
		//rad_five.setTag("S");
		rad_one.setText(getIntent().getStringExtra("A"));
		rad_two.setText(getIntent().getStringExtra("B"));*/
		/*rad_three.setText(getIntent().getStringExtra("C"));
		rad_four.setText(getIntent().getStringExtra("D"));*/
		//rad_five.setText(getIntent().getStringExtra("S"));
		...
		sonHandler=new Handler(){
			...
			public void handleMessage(Message msg) {
				...
				if(content!=null&&content.equals("sure")){//点击确定
					 if(bankCradParent.account!=null&&bankCradParent.account.length()>0){
					...
					if(CHOSE_DEVICE.equals("D")){
						if (Controller.isEmpty(ed_crad_pwd.getText().toString().trim())) {
						    Toast.makeText(getApplicationContext(), "输入密码不能为空", Toast.LENGTH_LONG).show();
							return;
						 }
						creditCardBean.transAmt="0";
						creditCardBean.acctNo=ed_crad.getText().toString().trim().replace(" ", "");
						submit();
					}else{
						bankCradParent.transAmt=price;
						bankCradParent.account=ed_crad.getText().toString().trim().replace(" ", "");
	...
	public void onClick(View v) {
		...
		if (v == btn_ok) {
			...
			if (btn_ok.getText().toString().trim().contains("获取卡号")) {
				...
				cardHeadDialog = new NotCardHeadDialog(QueryMoneyActivity.this,CHOSE_DEVICE);
				cardHeadDialog.show();
				if(CHOSE_DEVICE.equals("A")){
			...
			else if(btn_ok.getText().toString().trim().contains("查询")){			
				if (Controller.isEmpty(ed_crad.getText().toString().trim())) {
					Toast.makeText(getApplicationContext(), "卡号不能为空", Toast.LENGTH_LONG).show();
					return;
		...
		if (v == topUpDialog.txt_ok) {
			...
			if (CHOSE_DEVICE.equals("A")&&Controller.isEmpty(topUpDialog.ed_crad_pass.getText().toString().trim())) {
				Toast.makeText(QueryMoneyActivity.this, "密码不能为空", Toast.LENGTH_LONG).show();
				return;
			}
	...
	void submit(){
		...
		new Thread() {
			...
			public void run() {
				...
				String pin="";
				if(CHOSE_DEVICE.equals("A")||CHOSE_DEVICE.equals("D")){
					pin=ed_crad_pwd.getText().toString().trim();
				}
				...
				runOnUiThread(new Runnable(){
						...
						public void run() {
							...
							if(pg!=null){
								...
								pg.setMessage("正在查询请稍后");
								pg.show();
							}else{
								pg=new ProgressDialog(QueryMoneyActivity.this);
								pg.setCancelable(false);
								pg.setMessage("正在查询请稍后");
								pg.show();
							}
				    ...
				    msgBean = controller2.Query(creditCardBean);
					handleMsg(msgBean);
					runOnUiThread(new Runnable(){
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if (msgBean.RspCd.contains("00")) {
					    ...
					    new AlertDialog.Builder(QueryMoneyActivity.this)
					    .setTitle("提示")
					    .setMessage("您的余额为："+msgBean.Balance)
		...
		swipHanlder=new Handler(){
			...
			public void handleMessage(Message msg) {
				...
				if (swipResult != null) {
					   ...
					   // kzinfoTrack = Dump.getHexDump(swipResult.getExtInfo()).replaceAll(" ", "");
	...
	//蓝牙刷卡流程
	   void swipBlueTooth(){
		   ...
		   final SwipResult swipResult = controller.swipCardMe3X("请刷卡/插卡","0", new SimpleTransferListener(), 4*30000L, TimeUnit.MILLISECONDS,swipHanlder,needTime);
	...
	private void initMe3xDeviceController(DeviceConnParams params) {
		controller.init(QueryMoneyActivity.this, Common.ME11_DRIVER_NAME, params, new DeviceEventListener<ConnectionCloseEvent>() {
			@SuppressLint("NewApi")
			@Override
					...
					try {
						...
						if(DeviceConnState.CONNECTED != controller.getDeviceConnState()&&Controller.isEmpty(ed_crad_amount.getText().toString().trim())){
							...
							runOnUiThread(new Runnable(){
								...
								@Override
								public void run() {
									Toast.makeText(QueryMoneyActivity.this,"连接中断,请稍后重试",Toast.LENGTH_SHORT).show();
			...
			if (deviceInfo != null) {
				runOnUiThread(new Runnable(){
					...
					public void run() {
						...
						Toast.makeText(QueryMoneyActivity.this,"请刷卡或者插卡",Toast.LENGTH_SHORT).show();
				...
				runOnUiThread(new Runnable(){
					...
					public void run() {
						...
						Toast.makeText(QueryMoneyActivity.this,"获取信息失败",Toast.LENGTH_SHORT).show();
	...
	private void swiperCard() {
		try {
			...
			ME11SwipResult swipRslt = controller.swipCard("0", 4*30000L, TimeUnit.MILLISECONDS);
			...
			if (swipRslt == null) {
				...
				if (moduleType[0] == ModuleType.COMMON_ICCARD) {// ic卡插卡
					...
					bankCrad.accNoT3="";
					//Log.v("track1", kzinfoTrack.substring(16, kzinfoTrack.length()));
					bankCrad.ic = "0";
	...
	private class SimpleTransferListener implements TransferListener {
		...
		public void onEmvFinished(boolean arg0, EmvTransInfo context) throws Exception {
			...
			closed = true;
			Toast.makeText(QueryMoneyActivity.this,"onEmvFinished",Toast.LENGTH_SHORT).show(); 
		}
		...
		public void onRequestOnline(EmvTransController arg0, EmvTransInfo context) throws Exception {
			...
			//System.out.println(">>>>55域打包集合:"+ ISOUtils.hexString(tlvPackage.pack()));
				...
				//controller.startPininput(AccountInputType.USE_ACCT_HASH, swipRslt.getAccount().getAcctHashId(), 6, true, "消费金额为:" + amt.toString() + "\n请输入交易密码:", 60000);
				cardHeadDialog.dismiss();
		...
		public void onRequestPinEntry(EmvTransController arg0, EmvTransInfo arg1) throws Exception {
			...
			iHandler.sendMessage(msg);
			arg0.cancelEmv();
			Toast.makeText(QueryMoneyActivity.this,"onRequestPinEntry",Toast.LENGTH_SHORT).show();
		}
		...
		public void onRequestSelectApplication(EmvTransController arg0, EmvTransInfo arg1) throws Exception {
			...
			iHandler.sendMessage(msg);
			arg0.cancelEmv();
			Toast.makeText(QueryMoneyActivity.this,"onRequestSelectApplication",Toast.LENGTH_SHORT).show();
		}

		@Override
		public void onRequestTransferConfirm(EmvTransController arg0, EmvTransInfo arg1) throws Exception {
			Toast.makeText(QueryMoneyActivity.this,"onRequestTransferConfirm",Toast.LENGTH_SHORT).show();
		}

		@Override
		public void onSwipMagneticCard(SwipResult swipRslt) {
			Toast.makeText(QueryMoneyActivity.this,"onSwipMagneticCard",Toast.LENGTH_SHORT).show();
		}

		@Override
		public void onOpenCardreaderCanceled() {
			Toast.makeText(QueryMoneyActivity.this,"onOpenCardreaderCanceled",Toast.LENGTH_SHORT).show();
		}
	}
