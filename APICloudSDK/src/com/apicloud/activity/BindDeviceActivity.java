...
import com.apicloud.activity.card.CardPaymentsRules;
...
public class BindDeviceActivity extends BaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		bindAc=this;
		isBindPage=true;
		setContentView(UZResourcesIDFinder.getResLayoutID("bind_device"));
		initView();
		initHanlder();
		...
		controller2 = new Controller(BindDeviceActivity.this);
	...
	public void setCardNo(Object obj){
		...
		if(CHOSE_DEVICE.equals(UICommon.WFT_DEVICE)||CHOSE_DEVICE.equals(UICommon.LANDY_DEVICE)||CHOSE_DEVICE.equals(UICommon.CFT_DEVICE)||CHOSE_DEVICE.equals(UICommon.BBPOS_IC_DEVICE)){
			...
			Intent intent=new Intent(BindDeviceActivity.this,BindDeviceFinalActivity.class);
			csn=landybean.ksn+"";
			...
			startActivityForResult(intent,REQUEST_FOR_BIND);
		}
	...
	private void initView() {
		bind_lkey=this.getIntent().getStringExtra("lkey")+"";
		bind_shopno=this.getIntent().getStringExtra("shopno")+"";
		// TODO Auto-generated method stub
	...
	void initSwipParam(){
		fromAct=UICommon.BindDeviceAactivity;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(resultCode==BindDeviceFinalActivity.RESPONSE_FOR_BIND_FAIL||resultCode==BindDeviceFinalActivity.RESPONSE_FOR_BIND_SUCCESS){
			finish();
		}
	}
	/**
	 * 
	 * TODO返回给前端绑定结果，1表示成功，0表示失败
	   2015年12月7日
 RESPONSE_FOR_BIND_FAIL	   void
	 */
	public  void giveResultToHtml(int resultCode){
		Intent resultData = new Intent();
		JSONObject json = new JSONObject();
		//Tip(resultCode+"");
		try {
			json.put("callback", resultCode);
			Log.v("push1", resultCode+"");
			...
			public void handleMessage(Message msg) {
				...
				if(msg.obj!=null){
					...
					if (s.error.contains("0")) {
						Toast.makeText(BindDeviceActivity.this, "添加成功", Toast.LENGTH_LONG).show();
						...
						Toast.makeText(BindDeviceActivity.this, "该卡已存在", Toast.LENGTH_SHORT).show();
					}else  if(s.error.contains("1")){
						Toast.makeText(BindDeviceActivity.this, "添加失败", Toast.LENGTH_LONG).show();
					}else{
						Tip(s.error);
					}
				}
	...
	public void onClick(View v) {
		...
		// 业务规则跳转
		if (v == txt) {
			startActivity(new Intent(BindDeviceActivity.this, CardPaymentsRules.class));
		}
		// 获取卡号
		if (v == txt_crad) {
			cardHeadDialog = new NotCardHeadDialog(BindDeviceActivity.this,CHOSE_DEVICE);
			cardHeadDialog.show();
			operation_stay(Common.FETCH_DEVICE_INFO);
		}
		// 确定
		if (v == btn_ok) {
			if (Controller.isEmpty(ed_crad.getText().toString().trim())) {
				Toast.makeText(BindDeviceActivity.this, "卡号不能为空", Toast.LENGTH_LONG).show();
				return;
			}
			if (Controller.isEmpty(ed_crad_name.getText().toString().trim())) {
				Toast.makeText(BindDeviceActivity.this, "持卡人姓名不能为空", Toast.LENGTH_LONG).show();
				return;
			}
			if (b == false) {
				Toast.makeText(BindDeviceActivity.this, "还款业务规则必须同意", Toast.LENGTH_LONG).show();
				return;
			}
	...
	private void doOk() {
		...
		new Thread() {
			public void run() {
				state=controller2.pushCard(BindDeviceActivity.this, ed_crad.getText().toString().trim(), ed_crad_name.getText().toString().trim(),Controller.COMMON_ADD_CREADIT_CARD, getIntent().getStringExtra("uid"),null,null,null,null,resultHandler);
	...
	private void initMe3xDeviceController(DeviceConnParams params) {
		controller.init(BindDeviceActivity.this, Common.ME11_DRIVER_NAME, params, new DeviceEventListener<ConnectionCloseEvent>() {
			@SuppressLint("NewApi")
			@Override
	...
	public static final int REQUEST_FOR_BIND=200;//绑定设备请求
	/**
					...
					try {
						...
						if(DeviceConnState.CONNECTED != controller.getDeviceConnState()&&Controller.isEmpty(ed_crad.getText().toString().trim())){
							...
							runOnUiThread(new Runnable(){
								...
								@Override
								public void run() {
									//Toast.makeText(BindDeviceActivity.this,"连接中断,请稍后重试",Toast.LENGTH_SHORT).show();
			...
			if (deviceInfo != null) {
				runOnUiThread(new Runnable(){
					...
					public void run() {
						...
						Toast.makeText(BindDeviceActivity.this,"请刷卡或者插卡",Toast.LENGTH_SHORT).show();*/
