...
public class BaseActivity extends Activity {
	...
	// protected DHDeviceController
	// dhcontroller=DHDeviceControllerImpl.getInstance(CommunicationMode.BLUETOOTH_2Mode);
	 ...
	 * 默认蓝牙接受处理器
	 */
	public String CHOSE_DEVICE = "E";// 选择连接的设别类型
										// A表示卡头，B表示新大陆有键盘蓝牙，C表示鼎和蓝牙,D表示新大陆蓝牙无键盘
			...
			public void handleMessage(Message msg) {
				...
				if (msg.what == DHListener.CONNECT_SUCCESS) {
					...
					if (time != null && time.length() > 0) {
						...
						// Toast.makeText(BaseActivity.this, time,
		...
		if (address.length() > 0) {
			...
			try {
				...
				if (type.equals("B") || type.equals("D")) {
					...
					try {
						/*
						 * if(cardHeadDialog!=null){ cardHeadDialog.show(); }
						 */
