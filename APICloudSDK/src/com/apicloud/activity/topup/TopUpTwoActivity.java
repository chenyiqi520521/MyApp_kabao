 ...
 * @see 钱包充值界面
 */
public class TopUpTwoActivity extends BaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		int layoutId = UZResourcesIDFinder.getResLayoutID("top_up_two");
		if (layoutId > 0) {
			setContentView(layoutId);
		}
		initView();
		//code = getIntent().getStringExtra("code");
		topUpDialog = new TopUpDialog(TopUpTwoActivity.this);
	...
	public void onClick(View v) {
		...
		if (v == btn_ok) {
			...
			if (btn_ok.getText().toString().trim().contains("获取卡号")) {
				...
				cardHeadDialog = new NotCardHeadDialog(TopUpTwoActivity.this, CHOSE_DEVICE);
				cardHeadDialog.show();
				if (CHOSE_DEVICE.equals("A")) {
		...
		if (v == topUpDialog.txt_ok) {
			...
			/*if (CHOSE_DEVICE.equals("A") && Controller.isEmpty(topUpDialog.ed_crad_pass.getText().toString().trim())) {
				Toast.makeText(TopUpTwoActivity.this, "密码不能为空", Toast.LENGTH_LONG).show();
				return;
			}
	...
	void submit() {
		...
		new Thread() {
			...
			public void run() {
				...
				msgBean = controller2.consume(creditCardBean, code);
				handleMsg(msgBean);
				runOnUiThread(new Runnable() {
	...
	public void initMe3xDeviceController(DeviceConnParams params) {
		controller.init(TopUpTwoActivity.this, Common.ME11_DRIVER_NAME, params, new DeviceEventListener<ConnectionCloseEvent>() {
			@SuppressLint("NewApi")
			@Override
					...
					try {
						...
						if (DeviceConnState.CONNECTED != controller.getDeviceConnState() && Controller.isEmpty(ed_crad_amount.getText().toString().trim())) {
							...
							runOnUiThread(new Runnable() {
								...
								@Override
								public void run() {
									Toast.makeText(TopUpTwoActivity.this, "连接中断,请稍后重试", Toast.LENGTH_SHORT).show();
			...
			if (deviceInfo != null) {
				runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						Toast.makeText(TopUpTwoActivity.this, "请刷卡或者插卡", Toast.LENGTH_SHORT).show();
				...
				runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						Toast.makeText(TopUpTwoActivity.this, "获取信息失败", Toast.LENGTH_SHORT).show();
	...
	private class SimpleTransferListener implements TransferListener {
		...
		public void onEmvFinished(boolean arg0, EmvTransInfo context) throws Exception {
			...
			closed = true;
			Toast.makeText(TopUpTwoActivity.this, "onEmvFinished", Toast.LENGTH_SHORT).show();
		}
		...
		public void onRequestPinEntry(EmvTransController arg0, EmvTransInfo arg1) throws Exception {
			...
			iHandler.sendMessage(msg);
			arg0.cancelEmv();
			Toast.makeText(TopUpTwoActivity.this, "onRequestPinEntry", Toast.LENGTH_SHORT).show();
		}
		...
		public void onRequestSelectApplication(EmvTransController arg0, EmvTransInfo arg1) throws Exception {
			...
			iHandler.sendMessage(msg);
			arg0.cancelEmv();
			Toast.makeText(TopUpTwoActivity.this, "onRequestSelectApplication", Toast.LENGTH_SHORT).show();
		}

		@Override
		public void onRequestTransferConfirm(EmvTransController arg0, EmvTransInfo arg1) throws Exception {
			Toast.makeText(TopUpTwoActivity.this, "onRequestTransferConfirm", Toast.LENGTH_SHORT).show();
		}

		@Override
		public void onSwipMagneticCard(SwipResult swipRslt) {
			Toast.makeText(TopUpTwoActivity.this, "onSwipMagneticCard", Toast.LENGTH_SHORT).show();
		}

		@Override
		public void onOpenCardreaderCanceled() {
			Toast.makeText(TopUpTwoActivity.this, "onOpenCardreaderCanceled", Toast.LENGTH_SHORT).show();
		}
	}
