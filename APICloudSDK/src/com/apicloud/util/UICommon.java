package com.apicloud.util;


/**
 * 保存一些页面信息，比如页面序号
 * @author Administrator
 *
 */
public class UICommon {
	
	//页面序号
	public static int TopUpActivity=1;
	public static int AddCardActivity=2;
	public static int QueryMoneyAactivity=3;
	public static int AddCardPaymentActivity=4;
	public static int AddTimelyAccountActivity=5;
	public static int TimelyAaccountTopUp=6;
	public static int CardPayActivity=7;
	public static int TelPhoneActivity=8;
	public static int TradnsferAccountsActivity=9;
    
	public static int BindDeviceAactivity=10;
	public static int KuailainActivity=11;
	public static int AddNewCardActivity=12;
	public static int BindEquipmentActivity=13;
	public static int AddNewCreditActivity=14;
	public static int RealTimeActivity=15;
	public static int TopUpThreeActivity = 16;
	
	//设备编号
	//设备编号
	public static String LANDY_DEVICE="E";//联迪设备
	public static String AF_DEVICE="F";//安付设备
	public static String CFT_DEVICE="G";//财付通
    public static String BBPOS_IC_DEVICE="H";//带刷IC卡的BBPOS
    public static String WFT_DEVICE="I";//万富通设备
	

}
